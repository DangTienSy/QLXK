...
{
    public partial class FormBENXE : Form
    {
        ...
        private void FormBENXE_Load(object sender, EventArgs e)
        {
            LoadDataGridView();
            huy();
        }
        ...
        {
            dgv.DataSource = DataAccess.GetDataTable("select * from BENXE order by MABENXE");
        }
        ...
        private void huy()
        {
            txtDiaChi.Text = txtHotLine.Text = txttenbenxe.Text = "";
        ...
        private void btnthem_Click(object sender, EventArgs e)
        {
            ...
            string t = txttenbenxe.Text.ToUpper();
            if (t == "") t = "NULL"; else t = String.Format("N'{0}'", t);
            string dc = txtDiaChi.Text.ToUpper();
            if (dc == "") dc = "NULL"; else dc = String.Format("N'{0}'", dc);
            string hl = txtHotLine.Text.ToUpper();
            if (hl == "") hl = "NULL"; else hl = String.Format("N'{0}'", hl);
            string cmd = String.Format("insert into BENXE(tenBENXE, diachi,hotline) values ({0},{1},{2})", t,dc,hl);
            ...
            foreach (DataGridViewCell cell in dgv.SelectedCells)
                if (cell.Selected)
                {
                    try
                    {
                        ...
                        string cmd = String.Format("delete from BENXE WHERE MABENXE={0}", ma);
            ...
            if (dgv.SelectedCells.Count > 0) if (dgv.SelectedCells[0].Selected)
                {
                    ...
                    try
                    {
                        txttenbenxe.Text = dgv.Rows[i].Cells[1].Value.ToString();
                        txtDiaChi.Text = dgv.Rows[i].Cells[2].Value.ToString();
                        txtHotLine.Text = dgv.Rows[i].Cells[3].Value.ToString();
                    }
                    catch (Exception m)
                    {
                        MessageBox.Show(m.Message);
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            if (dgv.SelectedCells[0].Selected)
            {
                try
                {
                    ...
                    string cmd = String.Format("update BENXE set TENBENXE={0},DIACHI={1},HOTLINE={2} WHERE MABENXE={3}", t,dc,hl, ma);
                    if (DataAccess.Execute(cmd))
                    {
                        LoadDataGridView();
                        huy();
